  ...
  function createOutlineBoxes(outline, container) {
    ...
    generateTextButton.addEventListener('click', () => {
      ...
      if (textType === 'A') {
        ...
        if (outline) {
          const outlineString = outline.map(point => `${point.title}\n${point.content.join('\n')}`).join('\n\n');
          const generatedText = generateOutlineText(mainkeyword, subkeywords, proofkeywords, w_fragen, outlineString); // Generiere den Text mit der Gliederung
          insertTextAndSend(mainkeyword, generatedText, subkeywords, proofkeywords, w_fragen); // Text einfügen
        }
      } else if (textType === 'B') {
