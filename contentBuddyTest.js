(function() {
  ...
  function createOutlineBoxes(outline, container) {
    ...
    generateTextButton.addEventListener('click', () => {
      ...
      if (textType === 'A') {
        const outline = extractOutline(); // Extrahiere die Gliederung
        if (outline) {
          createOutlineBoxes(outline, container); // Erstelle die Gliederungsboxen
          generateOutlineAndText(mainkeyword, subkeywords, proofkeywords, w_fragen, outline); // Generiere den Text
        }
      } else if (textType === 'B') {
  ...
  function generateOutlineAndText(hauptkeyword, nebenkeywords, proofkeywords, w_fragen, outline) {
    const outlineText = outline.map(point => `${point.title}\n${point.content.join(' ')}`).join('\n\n');
    const text = window.promptTextOutline
      ...
      .replace(/\$\{outline\}/g, outlineText); // Füge die Gliederung hier hinzu
    insertTextAndSend(hauptkeyword, text, nebenkeywords, proofkeywords, w_fragen);
  }
      ...
      if (hauptkeyword) {
        if (textType === 'A') {
          ...
          setTimeout(() => {
            const outline = extractOutline(); // Extrahiere die Gliederung
            if (outline) {
              createOutlineBoxes(outline, content); // Erstelle die Gliederungsboxen
              generateOutlineAndText(hauptkeyword, nebenkeywords, proofkeywords, w_fragen, outline); // Generiere den Text
            }
          }, 10000); // Wartezeit anpassen, um sicherzustellen, dass die Gliederung erstellt wird
        } else if (textType === 'B') {
